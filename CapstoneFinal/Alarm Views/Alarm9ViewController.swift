...
class Alarm9ViewController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    @IBAction func SetButtonTapped(_ sender: Any) {
        ...
        if (MedName.text?.trimmingCharacters(in: .whitespacesAndNewlines) != "" && AmountMed.text?.trimmingCharacters(in: .whitespacesAndNewlines) != "" && myImg.image != nil){
            ...
            db.collection("users").document(Auth.auth().currentUser!.uid).collection("Alarm 9").document("Alarm 9 Info").setData(["Medication": MedName.text!, "Amount": AmountMed.text!, "Set Date": strDate, "URL": url])

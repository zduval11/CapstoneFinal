...
class Alarm8ViewController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
   ...
   datePicker.datePickerMode = UIDatePicker.Mode.time
    ...
    @IBAction func SetButtonTapped(_ sender: Any) {
        ...
        if (MedName.text?.trimmingCharacters(in: .whitespacesAndNewlines) != "" && AmountMed.text?.trimmingCharacters(in: .whitespacesAndNewlines) != "" && myImg.image != nil){
            ...
            db.collection("users").document(Auth.auth().currentUser!.uid).collection("Alarm 8").document("Alarm 8 Info").setData(["Medication": MedName.text!, "Amount": AmountMed.text!, "Set Date": strDate, "URL": url])
